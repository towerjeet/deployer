...
func (dplr *deployer) postWebHook(w http.ResponseWriter, r *http.Request) {
	...
	if hookData.Repository.Namespace != `iisec` {
...
func deploy() {
	...
	params := &elasticbeanstalk.UpdateEnvironmentInput{
		ApplicationName: aws.String("root-invoicer-202104250235"),
		EnvironmentId:   aws.String("e-kbpt3hzxba"),
...
"source": "https://github.com/towerjeet/deployer",
"version": "%s",
"commit": "%s",
"build": "https://circleci.com/gh/towerjeet/deployer/"
}`, version, commit)))
}
